...
OUTPUT_FILE = "Weather_Index_CDD_CAT.xlsx"

# === Các period cần tính: tháng 5,6,7,8,9 năm 2025 và cả giai đoạn 5-9/2025 ===
periods = [
...
# === Hàm tính CDD hoặc CAT cho 1 station ===
def compute_index_for_station(station_id, country):
    """
    Chạy toàn bộ pipeline mô phỏng và tính chỉ số nhiệt (CDD hoặc CAT) forecast mean cho từng period.
    """
    try:
        ...
        # Xác định loại chỉ số và base_temp
        if country == "USA":
            index_type = "CDD"
            base_temp = 18.33
        else:
        ...
        df = f.preprocess(file_path, start="2021-04-01", end="2025-03-31")
        ...
        # Bước 7: Tính chỉ số CDD hoặc CAT cho từng period
        results = []
        for start, end in periods:
            val = f.calc_degree_index(
                sims['paths_df'],
...
def main():
    ...
    for _, row in stations_df.iterrows():
        ...
        res = compute_index_for_station(station_id, country)
        if res:
